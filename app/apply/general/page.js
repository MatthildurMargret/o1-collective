...
export default function GeneralApplyPage() {
  const [form, setForm] = useState({
    name: '', email: '', company: '', role: '', linkedin: '', how_heard: '', referred_by: '', why: '',
  })
  ...
  async function handleSubmit(e) {
    ...
    const res = await fetch('/api/apply', {
      ...
      body: JSON.stringify({ type: 'member', ...form }),
    })
        ...
        {done ? (
          ...
          <>
            <div style={{ marginBottom: 52 }}>
              <p style={{ fontSize: 12, letterSpacing: '0.1em', color: '#A8A49C', textTransform: 'uppercase', margin: '0 0 16px' }}>General Membership</p>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 36 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 28 }}>
                <div>
                  ...
                  <input type="text" required value={form.role} onChange={update('role')} placeholder="Partner, Operator, Angel…" style={inputStyle} />
                </div>
                <div>
              ...
              <div>
                <label style={labelStyle}>How did you hear about O1 Collective?</label>
                <input type="text" value={form.how_heard} onChange={update('how_heard')} placeholder="Referral, event, social media…" style={inputStyle} />
              </div>
